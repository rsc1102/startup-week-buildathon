...
import ConfirmTrip from "../confirm_trip/confirm_trip";

export default function Home() {
  const [sourceDestinationConfirmed, setSourceDestinationConfirmed] =
    useState(false);
  ...
  function cancelRouteHandler() {
    if (!routeConfirmed) return;
    setRouteConfirmed(false);
  }

  function confirmTripHandler() {
  ...
  function updateMaxFareHandler(value: number) {
    if (value < 0) return;
    setMaxFare(Number(value));
  }
        ...
        >
          <div className="bg-slate-100/90 z-1 w-full flex flex-col justify-center items-center">
            ...
            <InputFields
              ...
              inputDisabled={routeConfirmed || tripConfirmed}
            />
          </div>
            ...
            !routeConfirmed ? (
              <div className="w-full flex flex-col items-center ">
                <Map
                  ...
                  sourceDestinationConfirmed={sourceDestinationConfirmed}
                  routeConfirmedCallback={routeConfirmedCallback}
                />
                {sourcePlace &&
                destinationPlace &&
                !sourceDestinationConfirmed ? (
                  <button
                    ...
                    className="border p-1 rounded-md flex m-1 w-50 items-center bg-lime-500 z-1 hover:bg-lime-600 active:bg-lime-600 cursor-pointer"
                  >
              ...
              <ConfirmTrip
                maxFare={maxFare}
                updateMaxFareHandler={updateMaxFareHandler}
                cancelRouteHandler={cancelRouteHandler}
                confirmTripHandler={confirmTripHandler}
              />
            )
          ) : (
            <UserBidingComponent
